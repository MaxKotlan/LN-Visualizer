...
const sortGraphByCentrality = (
  ...
  return g.sort(
    (a, b) => precomputedNodeEdgeList[b.pub_key].length - precomputedNodeEdgeList[a.pub_key].length,
  );
};
...
const calculateParentChildRelationship = (
  ...
  if (!maxConnNode || !maxConnEdge || maxConnNode?.connectedEdges.length <= n.connectedEdges.length)
    return;
