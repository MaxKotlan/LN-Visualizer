        ...
        () =>
            combineLatest([
                ...
                map(([filters, , nodeRegistry]) => {
                    if (!this.channelBufferService.vertex || !this.filteredNodeRegistry)
                        return null;
                    let i = 0;
                    this.filteredNodeRegistry.forEach((channel) => {
    ...
    channelColors$ = createEffect(
        () =>
            combineLatest([
                ...
                sampleTime(this.throttleTimeMs),
                map(([filters, , nodeRegistry]) => {
                    if (!this.channelBufferService.color || !this.filteredNodeRegistry) return null;
                    let i = 0;
                    this.filteredNodeRegistry.forEach((channel) => {
                        if (this.filterEvaluationService.evaluateFilters(channel, filters)) {
                            const node1 = nodeRegistry.nodeSet.get(channel.policies[0].public_key);
